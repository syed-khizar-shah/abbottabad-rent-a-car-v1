...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (heroImageFile && heroImageFile.size > 0) {
      ...
      if (
        existingContent?.heroImage &&
        existingContent.heroImage.startsWith("http")
      ) {
        await deleteImage(
          existingContent.heroImage,
          "abbottabad-rent-a-car/homepage"
        );
      }
      // Upload new image
    ...
    const content = await HomepageContent.findOneAndUpdate({}, updateData, {
      new: true,
      upsert: true,
    });
